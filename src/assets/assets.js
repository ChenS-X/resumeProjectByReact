...
export const projectsExpData = [
  ...
  },
  {
    ...
    link: "",
    content: [
      `背景：实时公交车辆及线路信息可视化，在页面上同时渲染3w+动态车辆，及其线路和场站围栏等信息。容易造成页面卡顿，网络请求耗时长等问题。`,
      `技术选型：采用”滑动窗口“的技术，对实时大数据分片，并发请求网络数据。模拟点聚合方式减少页面上同时渲染元素的数量。并使用<span class="text-red-500 bg-amber-200">Web Worker</span>辅助计算可视区域点位，减少主线程压力。`,
      `实现方案：监听地图移动&缩放zoom变化（地图采用minemap方案），动态划分地图区域网格，模拟点聚合方式，在每个区域中平均显示定量的公交点位。随着地图移动&缩放，每个区域点位越精准，直至用户可操作每个点位。采用<span class="text-red-500 bg-amber-200">Web Worker</span>辅助计算；使用<span class="text-red-500 bg-amber-200">RequestAnimationFrame（rAF）</span>结合时间切片<span style="font-weight: bold">Time Slicing</span>理念优化渲染。`,
      `可优化点：①采用权重计算，根据每个区域车辆比重动态划分区域展示数量；②服务端使用<span style="font-weight:bold">SSE</span>主动推送增量更新数据代替全量更新。`
    ],
  },
];
